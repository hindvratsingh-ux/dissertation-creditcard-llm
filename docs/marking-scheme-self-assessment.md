...
## Update (2026-07-31): re-verified against ALL six official documents, including two not previously checked this closely

Re-read "What is required for each dissertation type.pdf" (Type 5 section specifically) and the original supervisor-reviewed "Methodology_Draft_Hindvrat_updated.pdf" in full, and directly inspected the current `.tex` files rather than trusting earlier status claims. Findings:

**Claim "we're only waiting on the judge data, everything else is done" is not accurate.** Three literal `[NOTE TO SELF/SUPERVISOR]` placeholder markers are still sitting in the actual chapter files right now: `ch4_analysis.tex` (Section 4.5, explanation-quality/consistency results), `ch5_discussion.tex` (RQ4 answer), and `ch6_conclusion.tex` (RQ4 in the summary table). The consistency data (`results/consistency_scores.csv`) has been sitting complete and unused since two days ago — the raw data file was committed, but nobody ever went back and wrote the actual sentences using it into the chapters. This is fixable immediately and does not need to wait for the judge run.

**Declaration is still a placeholder**, unchanged since it was first flagged: still has its own `[NOTE TO SELF]` note, still missing the ethics-approval statement, archive-permission tickboxes, explicit word count, and dissertation-type confirmation that the official example template requires. Acknowledgements section also still has a placeholder needing the supervisor's actual name.

**New finding: Type 5's official guidance names four specific criteria your methodology/discussion should explicitly address by name — "Replicability, Reliability, Validity, Generalisability"** (from "What is required for each dissertation type.pdf", Type 5 section). The dissertation currently discusses the substance of all four scattered across different sections (e.g. consistency scoring is substantively a reliability check, the limitations section touches generalisability) but never names or organises around this exact four-part framework. Since this is explicit official guidance for Type 5 specifically, a marker familiar with this document may expect to see it addressed directly by name, not just implicitly.

**Minor discrepancy, likely harmless:** the original supervisor-reviewed methodology draft describes the profile calibration source as "a real UK dataset of 30,000 consumer records," while the current, actually-implemented code (`src/build_profiles.py`, verified directly) and the final dissertation cite the ONS Living Costs and Food Survey 2022–23 specifically by name. The code is unambiguous — it only ever references ONS, nothing else — so the dissertation is accurate to what was actually built. The original draft's phrasing was likely an early, looser description before the exact source was pinned down. Not a correctness problem, but worth being aware of in case your supervisor asks about it directly, since their copy of the record may still say "30,000 consumer records."

**No explicit hypotheses.** The Type 5 guidance says experiments "preferably involve hypothesis testing" (not mandatory) and gives an example of stating hypotheses upfront (e.g. "CAFS will be more efficient... for up to 250MB"). This dissertation has four research questions but no explicit a priori hypotheses. Since data collection is already complete, adding hypotheses now would risk looking like post-hoc rationalisation dressed up as prediction — better to leave as research questions and not manufacture hypotheses after the fact, but worth flagging as a design choice if asked.
